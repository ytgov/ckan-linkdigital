 ...
 */
ckan.module("yukon-tom-select", function () {
  ...
  return {
    ...
    // `this.sandbox.yukon.nestedOptions` and we receive
    // `{helloWorld: {byeWorld: ...}}`.
    options: {
    ...
    initialize() {
      ...
      if (typeof TomSelect === "undefined") {
        ...
        console.error("[yukon-tom-select] TomSelect library is not loaded");
        return
      }

      // tom-select has a number of nested options. We are using
      // `nestedOptions` helper defined inside `yukon.js` to
      // convert flat options of CKAN JS module into nested object.
      const options = this.sandbox["yukon"].nestedOptions(this.options);
