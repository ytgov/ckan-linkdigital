[![Tests](https://github.com//ckanext-yukon/workflows/tests.yml/badge.svg)](https://github.com//ckanext-yukon/actions/workflows/test.yml)
...
Code of the extension resides inside `ckanext/yukon`.
...
| Blanket             | Effect                                                                                              |
|---------------------|-----------------------------------------------------------------------------------------------------|
| actions             | Register all public functions from `ckanext.yukon.logic.action` as actions        |
| auth_functions      | Register all public functions from `ckanext.yukon.logic.auth` as auth functions   |
| blueprints          | Register all blueprints from `ckanext.yukon.views` as blueprints                  |
| cli                 | Register all public members(`__all__`) from `ckanext.yukon.cli` as commands       |
| config_declarations | Register all declarations from `ckanext/yukon/config_declaration.yaml`            |
| helpers             | Register all public functions from `ckanext.yukon.helpers` as helpers             |
| validators          | Register all public functions from `ckanext.yukon.logic.validators` as validators |
...
function defined inside `ckanext.yukon.logic.action` will be
...
class YukonPlugin(p.SingletonPlugin):
    ...
```
...
First, create a module inside `ckanext.yukon.implementations`
...
Re-export implementation from `ckanext/yukon/implementations/__init__.py`
...
the module. As result, only this group will be available as `ckan yukon` CLI command.
...
* config options can be accessed by shorter name: `option()` instead of
  `tk.config["ckanext.yukon.option.name"]`.
...
`assets/webassets.yml` and include this named asset into template using `{% asset "yukon/ASSET_NAME" %}` tag.
...
template for the blog page may be stored as `templates/yukon/blog/index.html`.
...
to generate migrations for the model using `ckan generate migration -p yukon -m "Migration message"` CLI command.
...
so we suggest leaving them inside `ckanext/yukon/schemas`.
...
`ckanext/yukon/tests` contains examples of tests for standard
...
`ckanext/yukon/tests/benchmarks` contains benchmarks. They
...
pytest --cov=ckanext.yukon

## generate HTML report at htmlcov/index.html
pytest --cov=ckanext.yukon --cov-report html
```
...
Run tests from `ckanext/yukon/tests/test_plugin.py`

```sh
pytest ckanext/yukon/tests/test_plugin.py
```
...
contains filepath, class and test name: `ckanext/yukon/tests/test_smth.py:TestSmth:test_smth`
